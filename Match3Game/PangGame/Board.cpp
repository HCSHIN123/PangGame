...
bool Board::CheckBoard()
{
	...
	for (int y = 0; y < m_iHeight; ++y)
	{
		for (int x = 0; x < m_iWidth; ++x)
		{
			if (m_Board[y][x] == nullptr || m_Board[y][x]->getBlockType() == WALL)
				continue;
			if (CheckBlock({ x,y }, m_Board[y][x]->getBlockType())) //매치된 블럭 검사, 처리
			{
				if (!isRemoveSpecial(m_Board[y][x]))
				{
...
bool Board::CanMatch()
{
	// 본인 기준 상하좌우중 하나라도 같다면 그 방향 기준으로 가능성 검사
	...
	for (int y = 0; y < m_iHeight; ++y)
	{
		for (int x = 0; x < m_iWidth; ++x)
		{
			...
			if ((int)CurType >= (int)ALL_NORMAL_BLOCKTYPE && CurType != WALL)
				return true;
			if (CurType == WALL)
				continue;

			if (x - NextDistance >= 0 && m_Board[y][x - NextDistance]->getBlockType() != WALL)  //왼쪽
			{
				if (CurType == m_Board[y][x - 1]->getBlockType()) // 현재블록과 다음블록이 같을때만 검사 
				{
					if (CheckMatch({ x - NextDistance - 1, y }, CurType)) // 더 왼쪽
						return true;

					if (CheckMatch({ x - NextDistance, y + 1 }, CurType)) // 아래
						return true;

					if (CheckMatch({ x - NextDistance, y - 1 }, CurType)) // 위
						return true;
				}

				if (m_Board[y][x - 1]->getBlockType() != WALL && CurType == m_Board[y][x - NextDistance]->getBlockType())//가운데 위아래
				{
					if (CheckMatch({ x - 1, y - 1 }, CurType))
						return true;
					if (CheckMatch({ x - 1, y + 1 }, CurType))
						return true;
				}
			...
			if (x + NextDistance < m_iWidth && m_Board[y][x + NextDistance]->getBlockType() != WALL) //오른쪽
			{
				if (CurType == m_Board[y][x + 1]->getBlockType())
				{
					if (CheckMatch({ x + NextDistance + 1, y }, CurType)) // 더 오른쪽
						return true;
			...
			if (y + NextDistance < m_iHeight && m_Board[y + NextDistance][x]->getBlockType() != WALL) // 아래
			{
				if (CurType == m_Board[y + 1][x]->getBlockType())
				{
					if (CheckMatch({ x + 1, y + NextDistance }, CurType)) // 오른쪽
						return true;

					if (CheckMatch({ x , y + NextDistance + 1 }, CurType)) //  더 아래
						return true;

					if (CheckMatch({ x - 1, y + NextDistance }, CurType)) // 왼쪽
						return true;
				}

				if (m_Board[y + 1][x]->getBlockType() != WALL && CurType == m_Board[y + NextDistance][x]->getBlockType() )//가운데 좌우
				{
					if (CheckMatch({ x - 1, y + 1 }, CurType))
						return true;
					if (CheckMatch({ x + 1, y + 1 }, CurType))
			...
			if (y - NextDistance >= 0 && m_Board[y - NextDistance][x]->getBlockType() != WALL) // 위
			{
				if (CurType == m_Board[y - 1][x]->getBlockType())
				{
					if (CheckMatch({ x + 1, y - NextDistance }, CurType)) // 오른쪽
					...
					if (CheckMatch({ x , y - NextDistance - 1 }, CurType)) //  더 위
						return true;
...
bool Board::CheckMatch(Vector2 _position, TEXTURE_TYPE _type)
{
	...
	if (m_Board[_position.m_iy][_position.m_ix]->getBlockType() == _type)
		return true;
	else
		return false;
}
