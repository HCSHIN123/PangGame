...
{
	// 보간으로 부드럽게 이동
	if (m_fRenderPosition.m_fy < static_cast<float>(m_TargetPosition.m_iy))			// 목표위치에 도달하지 못했다면
	{
		m_fWeight += TimerManager::GetInstance()->GetfDeltaTime() * m_fDropSpeed;	// 시간경과에 따라 가중치 증가
		m_fWeight = m_fWeight < 1.0f ? m_fWeight : 1.0f;							// 최대 가중치 1.0f으로 보정

		// 이동방식은 -y방향으로만 움직이므로 y값으로만 계산
		// 가중치가 1.0f가 될 때, 현재위치 = 원래위치 + 이동해야할거리가 되므로 목표위치에 도달
		// 현재위치 = 원래위치 + 이동해야할 거리(목표위치 - 현재위치) * 가중치
		m_fRenderPosition.m_fy = static_cast<float>(m_BoardPosition.m_iy) +	(static_cast<float>(m_TargetPosition.m_iy) - static_cast<float>(m_BoardPosition.m_iy)) * m_fWeight;
		SetRectPosition();

		return false;	//	false를 반환하여 계속 이동
	}

	// 목표위치에 도달했다면 위치 세부오차를 없애기 위해 정확한 위치를 재정의
	...
	m_fWeight = 0.0f; //  가중치 0.0f로 초기화
	return true;	  //  true를 반환하여 이동종료
}
